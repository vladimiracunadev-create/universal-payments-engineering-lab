...
|---|---|---|
| Markdown | Fuente única, revisable en GitHub. | [Abrir fuente .md](https://github.com/vladimiracunadev-create/universal-payments-engineering-lab/blob/main/docs/payment-methods/cases/payment-initiation.md) |
| HTML | Página generada automáticamente para navegar. | [Abrir en GitHub Pages](https://vladimiracunadev-create.github.io/universal-payments-engineering-lab/payment-methods/cases/payment-initiation.html) |
| PDF | Manual descargable; el índice lleva a este caso. | [Abrir PDF en este caso](https://vladimiracunadev-create.github.io/universal-payments-engineering-lab/downloads/universal-payments-engineering-lab.pdf#nameddest=case-payment-initiation) |

Los tres muestran el mismo catálogo. Markdown es la fuente; HTML y PDF son salidas generadas y verificadas.

## En una frase

**Sirve para:** Cobrar mediante link o solicitud compartible.

**Modelo mental:** Un link o solicitud conecta una orden con una experiencia de pago; no es el rail que liquida.

## Ejemplo concreto

<div class="case-example-grid">
<article><span>Situación</span><p>Un profesional envía un link por una factura de $75.000. La URL expira y sólo conecta la orden con el medio elegido; no demuestra el pago por sí sola.</p></article>
<article><span>Detrás de la pantalla</span><p>Crea orden firmada, expirable y de un solo uso. Cliente abre link y elige el rail final.</p></article>
<article><span>Se acepta como pagado cuando</span><p>Backend confirma por API/webhook del rail. Después: Atribuye liquidación a orden, campaña o factura.</p></article>
</div>
...
|---|---|---|
| 1. Inicio | Crea orden firmada, expirable y de un solo uso. | ¿Existe una referencia propia, monto, moneda y expiración? |
| 2. Proceso | Cliente abre link y elige el rail final. | ¿Qué sistema externo puede producir el efecto financiero? |
| 3. Confirmación | Backend confirma por API/webhook del rail. | ¿La evidencia viene de una API, webhook, banco o archivo autoritativo? |
| 4. Cierre | Atribuye liquidación a orden, campaña o factura. | ¿Ledger, proveedor y banco pueden explicar el mismo resultado? |

```mermaid
flowchart LR
  A["Inicio<br/>Crea orden firmada, expirable y de un solo uso."] --> B["Proceso<br/>Cliente abre link y elige el rail final."]
  B --> C["Confirmar<br/>Backend confirma por API/webhook del rail."]
  C --> D["Cerrar<br/>Atribuye liquidación a orden, campaña o factura."]
...
**Fallo característico:** Link reutilizado o phishing se bloquea por firma/dominio.
...
**Evidencia para considerarlo exitoso:** Orden, click/iniciación y confirmación unidos por una referencia no reutilizable.

**Construcción concreta:** Frontend de checkout + URL firmada + PSP + antifraude.
...
**Tecnologías del catálogo:** `url`, `qr`, `api`, `notifications`

**Operaciones cubiertas:** `payment-link`, `invoice`, `request-to-pay`, `deep-link`
...
- **Decisión:** Usa Links y solicitud de pago solo si su cobertura, experiencia, reversibilidad y conciliación resuelven tu caso; compara al menos dos proveedores antes de LIVE.
...
El DEMO permite observar el recorrido de **Links y solicitud de pago**, incluyendo éxito, timeout recuperado, evento duplicado y diferencia de conciliación. No contacta al proveedor, no valida credenciales, no certifica cumplimiento y no mueve dinero.
