...
|---|---|---|
| Markdown | Fuente única, revisable en GitHub. | [Abrir fuente .md](https://github.com/vladimiracunadev-create/universal-payments-engineering-lab/blob/main/docs/payment-methods/cases/qr.md) |
| HTML | Página generada automáticamente para navegar. | [Abrir en GitHub Pages](https://vladimiracunadev-create.github.io/universal-payments-engineering-lab/payment-methods/cases/qr.html) |
| PDF | Manual descargable; el índice lleva a este caso. | [Abrir PDF en este caso](https://vladimiracunadev-create.github.io/universal-payments-engineering-lab/downloads/universal-payments-engineering-lab.pdf#nameddest=case-qr) |

Los tres muestran el mismo catálogo. Markdown es la fuente; HTML y PDF son salidas generadas y verificadas.

## En una frase

**Sirve para:** Iniciar un pago escaneando un código.

**Modelo mental:** El QR transporta datos para iniciar; por sí solo no demuestra que exista pago.
...
|---|---|---|
| 1. Inicio | Genera QR firmado, único y expirable. | ¿Existe una referencia propia, monto, moneda y expiración? |
| 2. Proceso | App interpreta datos e inicia el rail subyacente. | ¿Qué sistema externo puede producir el efecto financiero? |
| 3. Confirmación | API/webhook del rail confirma; el QR no confirma. | ¿La evidencia viene de una API, webhook, banco o archivo autoritativo? |
| 4. Cierre | Conciliar referencia QR contra abono o adquirente. | ¿Ledger, proveedor y banco pueden explicar el mismo resultado? |

```mermaid
flowchart LR
  A["Inicio<br/>Genera QR firmado, único y expirable."] --> B["Proceso<br/>App interpreta datos e inicia el rail subyacente."]
  B --> C["Confirmar<br/>API/webhook del rail confirma; el QR no confirma."]
  C --> D["Cerrar<br/>Conciliar referencia QR contra abono o adquirente."]
...
**Fallo característico:** QR sustituido/expirado se rechaza; duplicado no reaplica.
...
**Evidencia para considerarlo exitoso:** Referencia QR única ligada a confirmación autoritativa.

**Construcción concreta:** Estándar QR + firma + backend + proveedor bancario o wallet.
...
**Tecnologías del catálogo:** `emv-qr`, `deep-link`, `crc`, `tokenization`

**Operaciones cubiertas:** `merchant-presented`, `consumer-presented`, `static`, `dynamic`
...
- **Decisión:** Usa Pagos QR solo si su cobertura, experiencia, reversibilidad y conciliación resuelven tu caso; compara al menos dos proveedores antes de LIVE.
...
El DEMO permite observar el recorrido de **Pagos QR**, incluyendo éxito, timeout recuperado, evento duplicado y diferencia de conciliación. No contacta al proveedor, no valida credenciales, no certifica cumplimiento y no mueve dinero.
