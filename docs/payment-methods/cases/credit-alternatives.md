...
|---|---|---|
| Markdown | Fuente única, revisable en GitHub. | [Abrir fuente .md](https://github.com/vladimiracunadev-create/universal-payments-engineering-lab/blob/main/docs/payment-methods/cases/credit-alternatives.md) |
| HTML | Página generada automáticamente para navegar. | [Abrir en GitHub Pages](https://vladimiracunadev-create.github.io/universal-payments-engineering-lab/payment-methods/cases/credit-alternatives.html) |
| PDF | Manual descargable; el índice lleva a este caso. | [Abrir PDF en este caso](https://vladimiracunadev-create.github.io/universal-payments-engineering-lab/downloads/universal-payments-engineering-lab.pdf#nameddest=case-credit-alternatives) |

Los tres muestran el mismo catálogo. Markdown es la fuente; HTML y PDF son salidas generadas y verificadas.

## En una frase

**Sirve para:** Compra financiada en cuotas por un tercero.

**Modelo mental:** Un financiador paga al comercio y el cliente adquiere una obligación en cuotas.

## Ejemplo concreto

<div class="case-example-grid">
<article><span>Situación</span><p>Una persona compra un teléfono en cuatro cuotas. El financiador evalúa y paga al comercio; el cliente acepta un contrato con costo total y calendario.</p></article>
<article><span>Detrás de la pantalla</span><p>Solicita evaluación y muestra costo total. Cliente consiente; financiador aprueba y paga comercio.</p></article>
<article><span>Se acepta como pagado cuando</span><p>API confirma contrato y desembolso. Después: Comercio concilia neto; financiador administra cuotas.</p></article>
</div>
...
|---|---|---|
| 1. Inicio | Solicita evaluación y muestra costo total. | ¿Existe una referencia propia, monto, moneda y expiración? |
| 2. Proceso | Cliente consiente; financiador aprueba y paga comercio. | ¿Qué sistema externo puede producir el efecto financiero? |
| 3. Confirmación | API confirma contrato y desembolso. | ¿La evidencia viene de una API, webhook, banco o archivo autoritativo? |
| 4. Cierre | Comercio concilia neto; financiador administra cuotas. | ¿Ledger, proveedor y banco pueden explicar el mismo resultado? |

```mermaid
flowchart LR
  A["Inicio<br/>Solicita evaluación y muestra costo total."] --> B["Proceso<br/>Cliente consiente; financiador aprueba y paga comercio."]
  B --> C["Confirmar<br/>API confirma contrato y desembolso."]
  C --> D["Cerrar<br/>Comercio concilia neto; financiador administra cuotas."]
...
**Fallo característico:** Rechazo, mora o devolución siguen flujos separados.
...
**Evidencia para considerarlo exitoso:** Contrato aceptado, desembolso al comercio y calendario contable.

**Construcción concreta:** Proveedor BNPL + API/SDK + consentimiento + regulación de crédito.
...
**Tecnologías del catálogo:** `underwriting`, `schedule`, `disclosures`, `autopay`

**Operaciones cubiertas:** `bnpl`, `pay-in-four`, `point-of-sale-loan`, `invoice-financing`
...
- **Decisión:** Usa BNPL y crédito alternativo solo si su cobertura, experiencia, reversibilidad y conciliación resuelven tu caso; compara al menos dos proveedores antes de LIVE.
...
El DEMO permite observar el recorrido de **BNPL y crédito alternativo**, incluyendo éxito, timeout recuperado, evento duplicado y diferencia de conciliación. No contacta al proveedor, no valida credenciales, no certifica cumplimiento y no mueve dinero.
