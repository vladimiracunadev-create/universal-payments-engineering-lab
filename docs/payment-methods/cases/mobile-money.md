...
[← Volver a la tabla](../END_TO_END_MATRIX.md) · [Ver esta guía .md en GitHub](https://github.com/vladimiracunadev-create/universal-payments-engineering-lab/blob/main/docs/payment-methods/cases/mobile-money.md)

## El mismo caso en tres formatos

| Formato | Para qué sirve | Enlace |
|---|---|---|
| Markdown | Fuente única, revisable en GitHub. | [Abrir fuente .md](https://github.com/vladimiracunadev-create/universal-payments-engineering-lab/blob/main/docs/payment-methods/cases/mobile-money.md) |
| HTML | Página generada automáticamente para navegar. | [Abrir en GitHub Pages](https://vladimiracunadev-create.github.io/universal-payments-engineering-lab/payment-methods/cases/mobile-money.html) |
| PDF | Manual descargable; el índice lleva a este caso. | [Abrir PDF en este caso](https://vladimiracunadev-create.github.io/universal-payments-engineering-lab/downloads/universal-payments-engineering-lab.pdf#nameddest=case-mobile-money) |

Los tres muestran el mismo catálogo. Markdown es la fuente; HTML y PDF son salidas generadas y verificadas.

## En una frase

**Sirve para:** Pago desde saldo móvil con red de agentes.

**Modelo mental:** El valor vive en una cuenta móvil y entra o sale mediante agentes o bancos.

## Ejemplo concreto

<div class="case-example-grid">
<article><span>Situación</span><p>Una persona envía el equivalente a $15.000 desde su wallet móvil a un comercio. El operador confirma y luego se cuadran saldos y comisión del agente.</p></article>
<article><span>Detrás de la pantalla</span><p>Identifica wallet, cliente y agente. Operador valida PIN/OTP y mueve saldo.</p></article>
<article><span>Se acepta como pagado cuando</span><p>Referencia del operador confirma el pago. Después: Saldos, comisiones y float de agentes se concilian.</p></article>
</div>
...
|---|---|---|
| 1. Inicio | Identifica wallet, cliente y agente. | ¿Existe una referencia propia, monto, moneda y expiración? |
| 2. Proceso | Operador valida PIN/OTP y mueve saldo. | ¿Qué sistema externo puede producir el efecto financiero? |
| 3. Confirmación | Referencia del operador confirma el pago. | ¿La evidencia viene de una API, webhook, banco o archivo autoritativo? |
| 4. Cierre | Saldos, comisiones y float de agentes se concilian. | ¿Ledger, proveedor y banco pueden explicar el mismo resultado? |

```mermaid
flowchart LR
  A["Inicio<br/>Identifica wallet, cliente y agente."] --> B["Proceso<br/>Operador valida PIN/OTP y mueve saldo."]
  B --> C["Confirmar<br/>Referencia del operador confirma el pago."]
  C --> D["Cerrar<br/>Saldos, comisiones y float de agentes se concilian."]
...
**Fallo característico:** Falta de liquidez o reversa abre una excepción trazable.
...
**Evidencia para considerarlo exitoso:** Referencia del operador y saldos de cliente, agente y tesorería cuadrados.

**Construcción concreta:** Contrato operador + API/USSD + KYC + gestión de agentes y float.
...
**Tecnologías del catálogo:** `ussd`, `sim-toolkit`, `app`, `agent-network`

**Operaciones cubiertas:** `cash-in`, `cash-out`, `p2p`, `merchant-payment`, `bill-pay`
...
- **Decisión:** Usa Mobile money solo si su cobertura, experiencia, reversibilidad y conciliación resuelven tu caso; compara al menos dos proveedores antes de LIVE.
...
El DEMO permite observar el recorrido de **Mobile money**, incluyendo éxito, timeout recuperado, evento duplicado y diferencia de conciliación. No contacta al proveedor, no valida credenciales, no certifica cumplimiento y no mueve dinero.
...
[Abrir Mobile money en la tabla](../END_TO_END_MATRIX.md) · [Configurar localhost](../../LOCALHOST_AND_CONFIGURATION.md)
