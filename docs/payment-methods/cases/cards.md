...
[← Volver a la tabla](../END_TO_END_MATRIX.md) · [Ver esta guía .md en GitHub](https://github.com/vladimiracunadev-create/universal-payments-engineering-lab/blob/main/docs/payment-methods/cases/cards.md)

## El mismo caso en tres formatos

| Formato | Para qué sirve | Enlace |
|---|---|---|
| Markdown | Fuente única, revisable en GitHub. | [Abrir fuente .md](https://github.com/vladimiracunadev-create/universal-payments-engineering-lab/blob/main/docs/payment-methods/cases/cards.md) |
| HTML | Página generada automáticamente para navegar. | [Abrir en GitHub Pages](https://vladimiracunadev-create.github.io/universal-payments-engineering-lab/payment-methods/cases/cards.html) |
| PDF | Manual descargable; el índice lleva a este caso. | [Abrir PDF en este caso](https://vladimiracunadev-create.github.io/universal-payments-engineering-lab/downloads/universal-payments-engineering-lab.pdf#nameddest=case-cards) |

Los tres muestran el mismo catálogo. Markdown es la fuente; HTML y PDF son salidas generadas y verificadas.

## En una frase

**Sirve para:** Pago inmediato con tarjeta física o digital.

**Modelo mental:** La tarjeta autoriza un cargo; captura, liquidación y chargeback son hechos posteriores.

## Ejemplo concreto

<div class="case-example-grid">
<article><span>Situación</span><p>Una tienda vende un notebook por $649.990. El cliente ve una aprobación inmediata, pero el comercio todavía debe capturar, liquidar y conciliar el abono neto.</p></article>
<article><span>Detrás de la pantalla</span><p>Backend crea order/payment intent. PSP tokeniza, autentica y autoriza; luego captura.</p></article>
<article><span>Se acepta como pagado cuando</span><p>Webhook o consulta confirma la captura. Después: Adquirente liquida; comercio concilia fees y disputas.</p></article>
</div>
...
|---|---|---|
| 1. Inicio | Backend crea order/payment intent. | ¿Existe una referencia propia, monto, moneda y expiración? |
| 2. Proceso | PSP tokeniza, autentica y autoriza; luego captura. | ¿Qué sistema externo puede producir el efecto financiero? |
| 3. Confirmación | Webhook o consulta confirma la captura. | ¿La evidencia viene de una API, webhook, banco o archivo autoritativo? |
| 4. Cierre | Adquirente liquida; comercio concilia fees y disputas. | ¿Ledger, proveedor y banco pueden explicar el mismo resultado? |

```mermaid
flowchart LR
  A["Inicio<br/>Backend crea order/payment intent."] --> B["Proceso<br/>PSP tokeniza, autentica y autoriza; luego captura."]
  B --> C["Confirmar<br/>Webhook o consulta confirma la captura."]
  C --> D["Cerrar<br/>Adquirente liquida; comercio concilia fees y disputas."]
...
**Fallo característico:** Timeout queda UNKNOWN; chargeback se registra por separado.
...
**Evidencia para considerarlo exitoso:** ID del proveedor, estado capturado, evento verificado y línea de settlement.

**Construcción concreta:** Checkout alojado + REST API + webhook + PostgreSQL + PCI DSS.
...
**Tecnologías del catálogo:** `iso8583`, `emv-chip`, `emv-contactless`, `3ds`, `tokenization`, `hsm`, `pci-dss`

**Operaciones cubiertas:** `credit`, `debit`, `prepaid`, `commercial`, `card-present`, `card-not-present`, `recurring`, `installments`
...
- **Decisión:** Usa Tarjetas solo si su cobertura, experiencia, reversibilidad y conciliación resuelven tu caso; compara al menos dos proveedores antes de LIVE.
...
El DEMO permite observar el recorrido de **Tarjetas**, incluyendo éxito, timeout recuperado, evento duplicado y diferencia de conciliación. No contacta al proveedor, no valida credenciales, no certifica cumplimiento y no mueve dinero.
...
[Abrir Tarjetas en la tabla](../END_TO_END_MATRIX.md) · [Configurar localhost](../../LOCALHOST_AND_CONFIGURATION.md)
