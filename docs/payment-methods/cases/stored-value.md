# 10. Saldo almacenado y gift cards

[← Volver a la tabla](../END_TO_END_MATRIX.md) · [Ver esta guía .md en GitHub](https://github.com/vladimiracunadev-create/universal-payments-engineering-lab/blob/main/docs/payment-methods/cases/stored-value.md)

## El mismo caso en tres formatos

| Formato | Para qué sirve | Enlace |
|---|---|---|
| Markdown | Fuente única, revisable en GitHub. | [Abrir fuente .md](https://github.com/vladimiracunadev-create/universal-payments-engineering-lab/blob/main/docs/payment-methods/cases/stored-value.md) |
| HTML | Página generada automáticamente para navegar. | [Abrir en GitHub Pages](https://vladimiracunadev-create.github.io/universal-payments-engineering-lab/payment-methods/cases/stored-value.html) |
| PDF | Manual descargable; el índice lleva a este caso. | [Abrir PDF en este caso](https://vladimiracunadev-create.github.io/universal-payments-engineering-lab/downloads/universal-payments-engineering-lab.pdf#nameddest=case-stored-value) |

Los tres muestran el mismo catálogo. Markdown es la fuente; HTML y PDF son salidas generadas y verificadas.

## En una frase

**Sirve para:** Gift card o saldo mantenido por el propio programa.

**Modelo mental:** El programa mantiene un saldo propio que debe reservarse y debitarse atómicamente.

## Ejemplo concreto

<div class="case-example-grid">
<article><span>Situación</span><p>Un cliente usa $8.000 de una gift card con saldo $10.000. El sistema reserva, captura y deja $2.000 sin permitir dos consumos simultáneos.</p></article>
<article><span>Detrás de la pantalla</span><p>Identifica cuenta y saldo disponible. Reserva atómica evita gastar dos veces.</p></article>
<article><span>Se acepta como pagado cuando</span><p>Captura consume saldo o libera la reserva. Después: Ledger demuestra saldo y pasivo del programa.</p></article>
</div>
...
|---|---|---|
| 1. Inicio | Identifica cuenta y saldo disponible. | ¿Existe una referencia propia, monto, moneda y expiración? |
| 2. Proceso | Reserva atómica evita gastar dos veces. | ¿Qué sistema externo puede producir el efecto financiero? |
| 3. Confirmación | Captura consume saldo o libera la reserva. | ¿La evidencia viene de una API, webhook, banco o archivo autoritativo? |
| 4. Cierre | Ledger demuestra saldo y pasivo del programa. | ¿Ledger, proveedor y banco pueden explicar el mismo resultado? |

```mermaid
flowchart LR
  A["Inicio<br/>Identifica cuenta y saldo disponible."] --> B["Proceso<br/>Reserva atómica evita gastar dos veces."]
  B --> C["Confirmar<br/>Captura consume saldo o libera la reserva."]
  C --> D["Cerrar<br/>Ledger demuestra saldo y pasivo del programa."]
...
**Fallo característico:** Carrera o timeout se resuelve por idempotencia y reserva expirable.
...
**Evidencia para considerarlo exitoso:** Saldo anterior + movimientos = saldo nuevo, con journal balanceado.

**Construcción concreta:** API propia + PostgreSQL transaccional + ledger + reglas regulatorias.
...
**Tecnologías del catálogo:** `ledger`, `safeguarding`, `tokenization`, `qr`, `nfc`

**Operaciones cubiertas:** `e-money`, `closed-loop-wallet`, `prepaid-account`, `gift-card`, `store-credit`, `loyalty`
...
- **Decisión:** Usa Saldo almacenado y gift cards solo si su cobertura, experiencia, reversibilidad y conciliación resuelven tu caso; compara al menos dos proveedores antes de LIVE.
...
El DEMO permite observar el recorrido de **Saldo almacenado y gift cards**, incluyendo éxito, timeout recuperado, evento duplicado y diferencia de conciliación. No contacta al proveedor, no valida credenciales, no certifica cumplimiento y no mueve dinero.
...
[Abrir Saldo almacenado y gift cards en la tabla](../END_TO_END_MATRIX.md) · [Configurar localhost](../../LOCALHOST_AND_CONFIGURATION.md)
