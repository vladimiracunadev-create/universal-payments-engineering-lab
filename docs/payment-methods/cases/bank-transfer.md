...
|---|---|---|
| Markdown | Fuente única, revisable en GitHub. | [Abrir fuente .md](https://github.com/vladimiracunadev-create/universal-payments-engineering-lab/blob/main/docs/payment-methods/cases/bank-transfer.md) |
| HTML | Página generada automáticamente para navegar. | [Abrir en GitHub Pages](https://vladimiracunadev-create.github.io/universal-payments-engineering-lab/payment-methods/cases/bank-transfer.html) |
| PDF | Manual descargable; el índice lleva a este caso. | [Abrir PDF en este caso](https://vladimiracunadev-create.github.io/universal-payments-engineering-lab/downloads/universal-payments-engineering-lab.pdf#nameddest=case-bank-transfer) |

Los tres muestran el mismo catálogo. Markdown es la fuente; HTML y PDF son salidas generadas y verificadas.

## En una frase

**Sirve para:** Pago directo entre cuentas bancarias.

**Modelo mental:** Una instrucción sale de una cuenta y llega a otra; el comprobante visual puede ser falso.

## Ejemplo concreto

<div class="case-example-grid">
<article><span>Situación</span><p>Un cliente transfiere $180.000 por una factura. La imagen del comprobante no sirve como confirmación: el abono debe aparecer en la API o cartola bancaria.</p></article>
<article><span>Detrás de la pantalla</span><p>Entrega beneficiario y referencia irrepetible. Cliente o API bancaria instruye transferencia.</p></article>
<article><span>Se acepta como pagado cuando</span><p>Cartola/API autoritativa confirma el abono. Después: Concilia monto, moneda, referencia y fecha valor.</p></article>
</div>
...
|---|---|---|
| 1. Inicio | Entrega beneficiario y referencia irrepetible. | ¿Existe una referencia propia, monto, moneda y expiración? |
| 2. Proceso | Cliente o API bancaria instruye transferencia. | ¿Qué sistema externo puede producir el efecto financiero? |
| 3. Confirmación | Cartola/API autoritativa confirma el abono. | ¿La evidencia viene de una API, webhook, banco o archivo autoritativo? |
| 4. Cierre | Concilia monto, moneda, referencia y fecha valor. | ¿Ledger, proveedor y banco pueden explicar el mismo resultado? |

```mermaid
flowchart LR
  A["Inicio<br/>Entrega beneficiario y referencia irrepetible."] --> B["Proceso<br/>Cliente o API bancaria instruye transferencia."]
  B --> C["Confirmar<br/>Cartola/API autoritativa confirma el abono."]
  C --> D["Cerrar<br/>Concilia monto, moneda, referencia y fecha valor."]
...
**Fallo característico:** Comprobante visual se ignora; diferencia queda pendiente.
...
**Evidencia para considerarlo exitoso:** Movimiento bancario autoritativo con cuenta, monto, moneda y referencia.

**Construcción concreta:** Cuenta empresa + API/open banking o archivos + conciliador.
...
**Tecnologías del catálogo:** `api`, `batch-sftp`, `iso20022`, `account-validation`

**Operaciones cubiertas:** `domestic`, `scheduled`, `wire`, `account-to-account`, `bulk-credit`
...
- **Decisión:** Usa Transferencia bancaria solo si su cobertura, experiencia, reversibilidad y conciliación resuelven tu caso; compara al menos dos proveedores antes de LIVE.
...
El DEMO permite observar el recorrido de **Transferencia bancaria**, incluyendo éxito, timeout recuperado, evento duplicado y diferencia de conciliación. No contacta al proveedor, no valida credenciales, no certifica cumplimiento y no mueve dinero.
