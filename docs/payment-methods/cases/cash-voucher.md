...
|---|---|---|
| Markdown | Fuente única, revisable en GitHub. | [Abrir fuente .md](https://github.com/vladimiracunadev-create/universal-payments-engineering-lab/blob/main/docs/payment-methods/cases/cash-voucher.md) |
| HTML | Página generada automáticamente para navegar. | [Abrir en GitHub Pages](https://vladimiracunadev-create.github.io/universal-payments-engineering-lab/payment-methods/cases/cash-voucher.html) |
| PDF | Manual descargable; el índice lleva a este caso. | [Abrir PDF en este caso](https://vladimiracunadev-create.github.io/universal-payments-engineering-lab/downloads/universal-payments-engineering-lab.pdf#nameddest=case-cash-voucher) |

Los tres muestran el mismo catálogo. Markdown es la fuente; HTML y PDF son salidas generadas y verificadas.

## En una frase

**Sirve para:** Orden digital pagada posteriormente en una red física.

**Modelo mental:** Una orden digital se paga físicamente; la confirmación llega después desde la red recaudadora.

## Ejemplo concreto

<div class="case-example-grid">
<article><span>Situación</span><p>Una tienda crea un voucher por $16.990 pagable en un recaudador. La orden queda pendiente hasta recibir el archivo o evento de la red física.</p></article>
<article><span>Detrás de la pantalla</span><p>Genera barcode/referencia y vencimiento. Recaudador recibe efectivo y reporta la operación.</p></article>
<article><span>Se acepta como pagado cuando</span><p>Archivo/API de la red confirma una sola vez. Después: Settlement del recaudador se concilia con voucher.</p></article>
</div>
...
|---|---|---|
| 1. Inicio | Genera barcode/referencia y vencimiento. | ¿Existe una referencia propia, monto, moneda y expiración? |
| 2. Proceso | Recaudador recibe efectivo y reporta la operación. | ¿Qué sistema externo puede producir el efecto financiero? |
| 3. Confirmación | Archivo/API de la red confirma una sola vez. | ¿La evidencia viene de una API, webhook, banco o archivo autoritativo? |
| 4. Cierre | Settlement del recaudador se concilia con voucher. | ¿Ledger, proveedor y banco pueden explicar el mismo resultado? |

```mermaid
flowchart LR
  A["Inicio<br/>Genera barcode/referencia y vencimiento."] --> B["Proceso<br/>Recaudador recibe efectivo y reporta la operación."]
  B --> C["Confirmar<br/>Archivo/API de la red confirma una sola vez."]
  C --> D["Cerrar<br/>Settlement del recaudador se concilia con voucher."]
...
**Fallo característico:** Voucher vencido/duplicado se rechaza o queda en excepción.
...
**Evidencia para considerarlo exitoso:** Referencia pagada en tienda y settlement del recaudador.

**Construcción concreta:** Contrato recaudador + barcode + API/archivo + conciliación diferida.
...
**Tecnologías del catálogo:** `barcode`, `qr`, `batch`, `webhook`

**Operaciones cubiertas:** `boleto`, `pay-at-store`, `barcode-voucher`, `collection-network`
...
- **Decisión:** Usa Voucher pagable en efectivo solo si su cobertura, experiencia, reversibilidad y conciliación resuelven tu caso; compara al menos dos proveedores antes de LIVE.
...
El DEMO permite observar el recorrido de **Voucher pagable en efectivo**, incluyendo éxito, timeout recuperado, evento duplicado y diferencia de conciliación. No contacta al proveedor, no valida credenciales, no certifica cumplimiento y no mueve dinero.
